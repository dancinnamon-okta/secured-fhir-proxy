...
module.exports.getUDAPConfiguration = (tenantConfig) => {
	...
	const communityKeyStore = udapCommon.parsePKCS12(path.resolve(configPath, 'udap_pki', tenantConfig.udap_pki_cert_filename),tenantConfig.udap_pki_cert_filename_pwd,'binary',false)
	...
	if (udapCommon.validateSanInCert(serverCertSAN, communityCertAndPrivateKey.certChain[0])) {
		var metadata = {
			...
			"signed_metadata": getSignedEndpointsJWT(communityCertAndPrivateKey, tenantConfig,process.env.BACKEND_SIGNING_ALGORITHM)
		}
...
function getSignedEndpointsJWT(certAndPrivateKey,tenantConfig,signingAlg) {
	const claims = {
