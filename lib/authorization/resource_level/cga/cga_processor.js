...
async function matchesIdOrSubject(tenantConfig, authorizedPrincipal, fhirObject) {
    ...
    console.log("According to the compartment, the following principals have access to this resource:")
    console.log(linkedPrincipals)
...
async function performBundleResourceCheck(tenantConfig, authorizationContext, fhirObject) {
    ...
    if(['patient','user'].includes(authorizationContext.authorizationMode) && fhirObject.entry) {
        var finalArray = []
