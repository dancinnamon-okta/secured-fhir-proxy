...
//const patientCompartment = require("../patient_compartment/patient_compartment")
...
async function performFGASingleResourceCheck(fgaClient, tenantConfig, authorizationContext, fhirObject, action) {
    ...
    const authorizedPrincipal = authorizationContext.externalAuthorizationPrincipal //authorizationContext.authorizationMode === "system" ? `external_entity:${authorizationContext.principalId}` : `user:${authorizationContext.principalId}`
    ...
    for(var i=0; i<linkedPatients.length - 1; i++) {
